        ...
        setDatatable({
          rows: res.data.map((da) => ({
            itemid:
              "DOM/" +
              `${
                da.Itemstatus.Itementry.Ledger.consumetype == "nonconsumable"
                  ? "NC"
                  : "C"
              }` +
              "/VOL" +
              da.Itemstatus.Itementry.Ledger.volumeno +
              "/PG" +
              da.Itemstatus.Itementry.Ledger.pageno +
              "/SNo" +
              da.Itemstatus.Itementry.Ledger.sno +
              "/" +
              da.Itemstatus.Itementry.createdAt.substring(0, 4) +
              "/" +
              da.Itemstatus.itemno +
              "/" +
              da.Itemstatus.Itementry.quantity,
            iname: da.Itemstatus.Itementry.Item.name,
            cate: da.Itemstatus.Itementry.Item.Category.name,
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              <h5 className="text-md font-bold">
                Designation: &nbsp;
                <span className="text-md font-normal">
                  {staffHistory[0].Staff.designation}
                </span>
              </h5>
            </div>
          </div>
                    ...
                    {staffHistory.map((staffhis, index) => (
                      ...
                      >
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                          DOM/
                          {staffhis.Itemstatus.Itementry.Ledger.consumetype ===
                          "nonconsumable"
                            ? "NC"
                            : "C"}
                          /VOL
                          ...
                          {staffhis.Itemstatus.Itementry.createdAt.substring(
                            0,
                            4
                          )}
                          /{staffhis.Itemstatus.itemno}/
                          {staffhis.Itemstatus.Itementry.quantity}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          {staffhis.Itemstatus.Itementry.Item.name}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {staffhis.Itemstatus.Itementry.Item.Category.name}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {staffhis.Location === null
                            ? "---"
                            : staffhis.Location.name}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {getStatusBadgeInTable(staffhis.paststatus)}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {staffhis.updatedAt.substring(0, 10)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
